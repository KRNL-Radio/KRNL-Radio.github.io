...
type RequestStatus = 'available' | 'temporary_throttled' | 'daily_throttled' | 'unavailable'
...
export default class RequestsCore extends Emitter {
    ...
    update_throttle_status() {
        ...
        let temporary_throttle = parser.parseExpression(`*/${this.period_length} * * * *`, {
            currentDate: this.last_request
        });
    ...
    async request(track_id: number) {
        const response = await fetch(`https://public.radio.co/stations/${this.station_id}/requests`, {
            method: 'POST',
            headers: {
    ...
    async get_tracks(): Promise<RequestTrack[]> {
        const response = await fetch(`https://public.radio.co/stations/${this.station_id}/requests/tracks`);
        const data = await response.json();
        return data.tracks;
    }
