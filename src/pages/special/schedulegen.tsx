...
function ScheduleGeneratorPage() {
    ...
    React.useEffect(() => {
        // get the duration
        let durationDT = DateTime.fromObject({ hour: duration.hour, minute: duration.minute })
    ...
    function startDateCb(e: any) {
        ...
        setDuration({
            ...duration,
            string: durationDT.toFormat("H:mm") + " (" + endDT.toFormat("hh:mm a") + ")"
        })
    }
    ...
    function durationCb(e: any) {
        ...
        let newTimeString = ""
        let dt = DateTime.fromObject({ hour: state.hour, minute: state.minute })
        let startDT = DateTime.fromObject({ hour: startTime.hour, minute: startTime.minute })
        let endDT = startDT.plus({ hours: state.hour, minutes: state.minute })

        newTimeString = dt.toFormat("H:mm") + " (" + endDT.toFormat("hh:mm a") + ")"
            ...
            <div className="flex flex-col items-center justify-center p-4">
                ...
                <div className="form-control">
                    ...
                    <label className="label cursor-pointer">
                        ...
                        <input type="checkbox" name="sunday" checked={dates.sunday} onChange={dateCheckboxCb} className="checkbox checkbox-primary" />
                    </label>
                    <label className="label cursor-pointer">
                        <span className="label-text">Monday</span>
                        <input type="checkbox" name="monday" checked={dates.monday} onChange={dateCheckboxCb} className="checkbox checkbox-primary" />
                    </label>
                    <label className="label cursor-pointer">
                        <span className="label-text">Tuesday</span>
                        <input type="checkbox" name="tuesday" checked={dates.tuesday} onChange={dateCheckboxCb} className="checkbox checkbox-primary" />
                    </label>
                    <label className="label cursor-pointer">
                        <span className="label-text">Wednesday</span>
                        <input type="checkbox" name="wednesday" checked={dates.wednesday} onChange={dateCheckboxCb} className="checkbox checkbox-primary" />
                    </label>
                    <label className="label cursor-pointer">
                        <span className="label-text">Thursday</span>
                        <input type="checkbox" name="thursday" checked={dates.thursday} onChange={dateCheckboxCb} className="checkbox checkbox-primary" />
                    </label>
                    <label className="label cursor-pointer">
                        <span className="label-text">Friday</span>
                        <input type="checkbox" name="friday" checked={dates.friday} onChange={dateCheckboxCb} className="checkbox checkbox-primary" />
                    </label>
                    <label className="label cursor-pointer">
                        <span className="label-text">Saturday</span>
                        <input type="checkbox" name="saturday" checked={dates.saturday} onChange={dateCheckboxCb} className="checkbox checkbox-primary" />
                    </label>
                    </div>
                    <h2 className="text-2xl font-bold">Start Time</h2>
                    <div>
                        <input type="range" min="0" max="23" name="hour" value={startTime.hour} onChange={startDateCb} className="range" />
                        <input type="range" min="0" max="55" step="5" name="minute" value={startTime.minute} onChange={startDateCb} className="range" />
                        <div>{startTime.string}</div>
                    </div>
                    <h2 className="text-2xl font-bold">Duration</h2>
                    <div>
                        <input type="range" min="0" max="23" name="hour" value={duration.hour} onChange={durationCb} className="range" />
                        <input type="range" min="0" max="55" step="5" name="minute" value={duration.minute} onChange={durationCb} className="range" />
                        <div>{duration.string}</div>
                    </div>
                    <h2 className="text-2xl font-bold">Output</h2>
                    <div>
                        <textarea className="textarea w-64 h-64 textarea-bordered textarea-primary" value={output} readOnly></textarea>
                    </div>
                </div>
            </div>
        </div>
