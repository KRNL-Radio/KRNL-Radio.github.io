...
function FullPlayer() {
    ...
    const [albumArt, setAlbumArt] = React.useState("https://placekitten.com/512/512");
        ...
        <div className="flex h-screen flex-col">
            ...
            <div className="flex flex-grow w-full">
                <Suspense fallback={LargeLoading()}>
                    {isSafari ? <div className="w-full h-full bg-gradient-to-r from-green-300 via-blue-500 to-purple-600"></div> : <Particles
                        id="tsparticles"
                        init={particlesInit}
                    ...
                    <div className="flex flex-col text-center bg-indigo-800 bg-opacity-40 w-1/2 h-1/2 absolute top-1/4 left-1/4 rounded-lg p-4 backdrop-blur-sm">
                        ...
                        <div className="text-xl text-white" ref={hostRef} >{host}</div>
                        {/* <button onClick={() => {
                        ...
                        <input type="checkbox" id="volume-modal" className="modal-toggle" data-theme="luxury" />
                        <div className="modal modal-bottom sm:modal-middle" data-theme="luxury">
                            <div className="modal-box">
                                <div className="flex flex-row items-center">
                                    <FontAwesomeIcon icon={solid("volume-down")} className="text-white" />
                                    <input className="range p-2" type="range" min="0" max="1" step="0.01" defaultValue={window.player.get_volume} onChange={(e) => {
                                        window.player.volume(parseFloat(e.target.value))
                                    }} />
                                    <FontAwesomeIcon icon={solid("volume-up")} className="text-white" />
                                </div>
                                <div className="modal-action">
                                    <label htmlFor="volume-modal" className="btn">Close</label>
                                </div>
                            </div>
                        </div>
                    </div>
