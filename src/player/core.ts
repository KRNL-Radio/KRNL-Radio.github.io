...
export class PlayerCore extends Emitter {
    ...
    constructor(station_id: string) {
        ...
        Promise.allSettled([this._refresh_metadata(), this._refresh_formats(), this._refresh_track_data()]).catch(console.error);
    }

    async refresh() {
        await Promise.allSettled([this._refresh_metadata(), this._refresh_formats(), this._refresh_track_data()])
    }
    ...
    async _refresh_formats() {
        let data = await (await fetch(`https://public.radio.co/stations/${this.station_id}/status`)).json()
        ...
        this.desktop_format = this.formats!.find((format) => format.format.toLowerCase() === "mp3");
        this.mobile_format = this.formats!.find((format) => format.format.toLowerCase() === "aac");
    }
    ...
    _refresh_browser_metadata() {
        if ('mediaSession' in navigator && this._audio) {
            ...
            navigator.mediaSession.metadata = new MediaMetadata({
                ...
                artwork: [
                    { src: this.player_data?.current_track.album_art ?? 'https://placekitten.com/600/600', sizes: '600x600', type: 'image/jpg' },
